...
public class ConvolutionalNeuralNet {
	...
	protected int[][][] sizes;
	protected int[] kernelSizes;
	protected int[][] strideLengths;
	protected int[][] featureMapRadii;
	...
	public static final class ConvolutionalLayer {
		private final int size;
		private final int strideLength;
		private final int featureMapRadius;

		public ConvolutionalLayer(int size, int featureMapRadius, int strideLength) {
			this.size = size;
			this.featureMapRadius = featureMapRadius;
		...
		protected int getStrideLength() {
			return this.strideLength;
		}

		protected int getFeatureMapRadius() {
			return this.featureMapRadius;
		}

//		public double[][][] feedForward(double[][][] input) {
//			double[][][] ret = new double[this.size][][];
//			for (int i=ret.length-1;i>=0;i--) {
//				ret[i] = this.convolute(input[0]);
//				for (int j=input.length-1;j>0;j--) {
//					ret[i] = VecMath.add(ret[i], this.convolute(input[j]));
//				}
//			}
//			return ret;
//		}
//
//		private double[][] convolute(double[][] input) {
//			double[][] ret = new double[input.length-this.featureMapRadius*2][];
//			for (int i=ret.length-1;i>=0;i--) {
//				ret[i] = new double[input[i].length-this.featureMapRadius*2];
//				for (int j=ret[i].length-1;j>=0;j--) {
//					double[][] tmp = new double[this.weights.length][];
//					for (int k=tmp.length-1;k>=0;k--) {
//						tmp[k] = new double[this.weights[k].length];
//						System.arraycopy(input[k-this.featureMapRadius], j-this.featureMapRadius, tmp[k], 0, tmp[k].length);
//					}
//					ret[i][j] = VecMath.sum(VecMath.multiply(tmp, this.weights[j]))+this.bias;
//				}
//			}
//			return ret;
//		}
	...
	public static final class ConvolutionalPooling {
		private final int kernelSize;
		private int[] sizes;
		private int[] strideLengths;
		private int[] featureMapRadii;

		public ConvolutionalPooling(int kernelSize, ConvolutionalLayer...convolutionalLayers) {
			this.kernelSize = kernelSize;
			this.sizes = new int[convolutionalLayers.length];
			this.strideLengths = new int[convolutionalLayers.length];
			this.featureMapRadii = new int[convolutionalLayers.length];
			for (int i=convolutionalLayers.length-1;i>=0;i--) {
				this.sizes[i] = convolutionalLayers[i].getSize();
				this.strideLengths[i] = convolutionalLayers[i].getStrideLength();
				this.featureMapRadii[i] = convolutionalLayers[i].getFeatureMapRadius();
			}
		}
		...
		protected int[] getStrideLengths() {
			return this.strideLengths;
		}

		protected int[] getFeatureMapRadii() {
			return this.featureMapRadii;
		}
	}

	/**
	 * Internal class to monitor the Training of a {@link BNN.ConvolutionalNeuralNet ConvolutionalNeuralNet}.
	 *
	 * @author Mario Schaeper
	 */
	public final class Monitor extends Observable {
		 ...
		 * <li>{@link BNN.ConvolutionalNeuralNet NeuralNet}.{@link BNN.ConvolutionalNeuralNet.Monitor Monitor}.
	...
	public ConvolutionalNeuralNet(int inputChannels, int inputWidth, int inputHeight,
			int fullyConnectedSize, int outputs, ConvolutionalPooling...layers) {
		int curImageWidth = inputWidth;
		int curImageHeight = inputHeight;
		this.inputWidth = inputWidth;
		this.inputHeight = inputHeight;
		this.sizes = new int[][][] {{{inputChannels}}, new int[layers.length][], {{fullyConnectedSize, outputs}}};
		this.kernelSizes = new int[layers.length];
		this.strideLengths = new int[layers.length][];
		this.featureMapRadii = new int[layers.length][];
		this.biases = new double[][][][] {new double[layers.length][][],
		...
		for (int i=0;i<layers.length;i++) {
			...
			this.featureMapRadii[i] = layers[i].getFeatureMapRadii();
		...
		for (int i=layers.length-1;i>=0;i--) {
			for (int j=fullyConnectedSize-1;j>=0;j--) {
				this.biases[1][0][0][j] = Math.random()*2 - 1;
				this.weights[1][0][0][0][0][j] = new double[curImageWidth * curImageHeight
	...
	public int getLayerAmount() {
		return Arrays.stream(this.sizes).mapToInt(
					i -> Arrays.stream(i).mapToInt(
						j -> j.length
					).sum()
				).sum() + this.sizes[1].length;
	}
	 ...
	 */
	public int getLayerSize(int layer) throws IndexOutOfBoundsException {
		if (layer < 0 || layer >= this.getLayerAmount()) {
			throw new IndexOutOfBoundsException("Layer does not exist");
		}
		int firstLayer = 0;
		int secondLayer = 0;
		for (int i=0;i<layer;i++) {
			if (secondLayer++ >= this.sizes[firstLayer].length) {
				secondLayer = 0 * firstLayer++;
			}
		}
		return this.sizes[firstLayer][secondLayer].length;
	}
	...
	public int getOutputSize() {
		int firstLayer = this.sizes.length-1;
		int secondLayer = this.sizes[firstLayer].length-1;
		int thirdLayer = this.sizes[firstLayer][secondLayer].length-1;
		return this.sizes[firstLayer][secondLayer][thirdLayer];
	}
	 ...
	 */
	public double getBias(int layer, int element) throws IndexOutOfBoundsException {
		if (layer < 0 || layer >= this.getLayerAmount()-1) {
			throw new IndexOutOfBoundsException("Layer does not exist or has no biases");
		}
		int firstLayer = 0;
		int secondLayer = 0;
		int thirdLayer = 0;
		for (int i=0;i<layer;i++) {
			if (thirdLayer++ >= this.biases[firstLayer][secondLayer].length) {
				thirdLayer = 0 * secondLayer++;
				if (secondLayer >= this.biases[firstLayer].length) {
					secondLayer = 0 * firstLayer++;
				}
			}
		}
		return this.biases[firstLayer][secondLayer][thirdLayer][element];
	}
	 ...
	 */
//	public double getWeight(int layer, int neuron, int neuronFrom) throws IndexOutOfBoundsException {
//		return this.weights[firstLayer];
//	}
			...
			for (int j=0;j<this.sizes[1][i].length;j++) {
				...
				for (int k=tmp.length-1;k>=0;k--) {
					...
					for (int l=tmp[k].length-1;l>=0;l--) {
						tmp[k][l] = new double[curImageHeight];
						for (int m=tmp[k][l].length-1;m>=0;m--) {
							tmp[k][l][m] = this.biases[0][i][j][k];
				...
				for (int k=tmp[j].length-1;k>=0;k--) {
					...
					for (int l=tmp[j][k].length-1;l>=0;l--) {
						...
						for (int m=this.kernelSizes[i]-1;m>=0;m--) {
							for (int n=this.kernelSizes[i]-1;n>=0;n--) {
								max = Math.max(max, retTmp[j][m][n]);
							}
						}
						tmp[j][k][l] = max;
					}
...
//	 * </li><li>regularization: 5</li><li>costFunction: {@link BNN.ConvolutionalNeuralNet.CrossEntropy CrossEntropy}</li></ul>
//	 *
...
//	 * @return the loaded {@link ConvolutionalNeuralNet AdvancedNet} Object
//	 * @throws IOException if a I/O error occours
//	 */
//	public static ConvolutionalNeuralNet loadFromFile(String file) throws IOException {
//		if (file == null) {
//			throw new IllegalArgumentException("File can not be null");
//		}
//		RandomAccessFile stream = new RandomAccessFile(file, "rw");
...
//		stream.close();
//		channel.close();
//		return new ConvolutionalNeuralNet(sizes, biases, weights);
//	}
}

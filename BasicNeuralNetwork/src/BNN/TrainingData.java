...
public class TrainingData {
	private static final int INPUT_START = -2;
	private static final int OUTPUT_START = -3;
	private static final int DATASET_END = -4;
	...
	public void saveToFile(String file, boolean overwrite) throws IOException {
		...
		stream.writeDouble(INPUT_START);
		for (int i=0;i<this.inputs.length;i++) {
			stream.writeDouble(this.inputs[i]);
		}
		stream.writeDouble(OUTPUT_START);
		for (int i=0;i<this.outputs.length;i++) {
			stream.writeDouble(this.outputs[i]);
		}
		stream.writeDouble(DATASET_END);
		lock.release();
		stream.close();
		channel.close();
	}

	public static TrainingData[] loadFromFile(String file) throws IOException {
		...
		channel.close();
		if (state != DATASET_END) {
			throw new IOException("File seems to be corrupt");
		}
		return fileData;
	}

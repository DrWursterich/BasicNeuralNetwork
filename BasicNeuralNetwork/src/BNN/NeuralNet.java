 ...
 * It contains methods to train a network of variable sizes with different
 * costfunctions. It is also possible to monitor various parts of the training
 * process.
 *
...
public class NeuralNet {
	...
	/**
	 * Internal class to monitor the Training of a
	 * {@link BNN.NeuralNet NeuralNet}.
	 *
	 * @author Mario Schaeper
	 */
	public final class Monitor extends Observable {
		...
		/**
		 * Internal class that represents a monitored change. The index is the
		 * value that changed:
		 * <ul>
		 * <li>{@link BNN.NeuralNet NeuralNet}.{@link BNN.NeuralNet.Monitor
		 * Monitor}. {@link BNN.NeuralNet.Monitor.TRAINING_COST
		 * TRAINING_COST}</li>
		 * <li>{@link BNN.NeuralNet NeuralNet}.{@link BNN.NeuralNet.Monitor
		 * Monitor}. {@link BNN.NeuralNet.Monitor.TRAINING_ACCURACY
		 * TRAINING_ACCURACY}</li>
		 * <li>{@link BNN.NeuralNet NeuralNet}.{@link BNN.NeuralNet.Monitor
		 * Monitor}. {@link BNN.NeuralNet.Monitor.EVALUATION_COST
		 * EVALUATION_COST}</li>
		 * <li>{@link BNN.NeuralNet NeuralNet}.{@link BNN.NeuralNet.Monitor
		 * Monitor}. {@link BNN.NeuralNet.Monitor.EVALUATION_ACCURACY
		 * EVALUATION_ACCURACY}</li>
		 * </ul>
		 *
		 * @author Mario Schaeper
		 */
		public final class Change {
			...
			/**
			 * Invokes a Change object with an index of the monitored change and
			 * the new value. The index should be one of the existing:
			 * <ul>
			 * <li>{@link BNN.NeuralNet NeuralNet}.{@link BNN.NeuralNet.Monitor
			 * Monitor}. {@link BNN.NeuralNet.Monitor.TRAINING_COST
			 * TRAINING_COST}</li>
			...
			/**
			 * Returns the index of the change. Preferably one of these:
			 * <ul>
			 * <li>{@link BNN.NeuralNet NeuralNet}.{@link BNN.NeuralNet.Monitor
			 * Monitor}. {@link BNN.NeuralNet.Monitor.TRAINING_COST
			 * TRAINING_COST}</li>
	 ...
	 * Improves the learningrate for very high differences between neuron
	 * outputs and ideal-values, making it the better choice most of the time.
	 *
	 * @author Mario Schaeper
	 */
	public final class CrossEntropy implements CostFunction {
		...
		public double fn(double[] outputActivations, double[] idealOutput) {
			return VecMath.sum(
					VecMath.subtract(
							VecMath.multiply(VecMath.multiply(idealOutput, -1), VecMath.log(outputActivations)),
							VecMath.multiply(
									VecMath.add(VecMath.multiply(outputActivations, -1), 1),
									VecMath.log(VecMath.add(VecMath.multiply(outputActivations, -1), 1)))));
		}
	}

	/**
	 * Creates a Neural Net with the given sizes. Biases have to match the
	 * values in the size array without the input nodes and weights are the
	 * matrices between the layers.<br>
	 * <em>e.g. size = {10, 6, 2}, biases = {double[6], double[2]}, weights =
	 * {double[10][6], double[6][2]}</em>
	 *
	 * @param size
			...
			for (int j = this.sizes[i] - 1; j >= 0; j--) {
				if (size[i - 1] != weights[i][j].length) {
					throw new IllegalArgumentException(
							"Weights length does not match sizes in layer " + i + " for neuron " + j);
				}
	...
	/**
	 * Creates a Neural Net with the given sizes. Biases and are initialized
	 * randomly between -1 an 1. Weights are distributed evenly across each
	 * layer.
	 *
	 * @param size
	 *            Array of sizes for each layer
	...
	/**
	 * Returns a weight of a connection between two neurons. The Layer has to be
	 * the one, in which the receiving neuron is.
	 *
	 * @param layer
	 *            the layer of the neuron the connection is to
	 * @param neuron
	 *            the neuron in the layer
	 * @param neuronFrom
	 *            the neuron of the previous layer, where the connection comes
	 *            from
	...
	public double getWeight(int layer, int neuron, int neuronFrom) {
		if (layer <= 0 || layer >= this.sizes.length) {
			throw new IllegalArgumentException(
					"Layer " + layer + " does not exist or has no connections to previous neurons");
		}
		if (neuron < 0 || neuron >= this.sizes[layer]) {
			throw new IllegalArgumentException("Neuron " + neuron + " does not exist in layer " + layer);
		}
	...
	/**
	 * Changes the monitoring properties. Every state set to <b>true</b> will
	 * cause training methods to print the according values after each
	 * iteration.
	 *
	 * @param trainingCost
	 *            state of monitoring the training cost
	 * @param trainingAccuracy
	 *            state of monitoring the training accuracy
	 * @param evaluationCost
	 *            state of monitoring the evaluation cost
	 * @param evaluationAccuracy
	 *            state of monitoring the evaluation accuracy
	 */
	public void setMonitoring(boolean trainingCost, boolean trainingAccuracy, boolean evaluationCost,
			boolean evaluationAccuracy) {
	 ...
	 *
	 * @param input
	 *            array of values used as input for the network
	...
	protected void stochasticGradientDescentCheckArguments(double[][][] trainingData, int epochs, int miniBatchSize,
		...
		stochasticGradientDescentCheckArguments(
				trainingData,
				epochs,
				miniBatchSize,
				learningRate,
		...
		for (int i = testData.length - 1; i >= 0; i--) {
			...
			if (testData[i][INPUT].length != this.sizes[0]) {
				throw new IllegalArgumentException(
						"Input size does not match amount of input-neurons in testData set " + i);
			}
			if (testData[i][OUTPUT].length != this.sizes[this.sizes.length - 1]) {
				throw new IllegalArgumentException(
						"Output size does not match amount of output-neurons in testData set " + i);
			}
		}
	}
	...
	protected void stochasticGradientDescentCheckArguments(double[][][] trainingData, int epochs, int miniBatchSize,
		...
		for (int i = trainingData.length - 1; i >= 0; i--) {
			...
			if (trainingData[i][INPUT].length != this.sizes[0]) {
				throw new IllegalArgumentException(
						"Input size does not match amount of input-neurons in trainingData set " + i);
			}
			if (trainingData[i][OUTPUT].length != this.sizes[this.sizes.length - 1]) {
				throw new IllegalArgumentException(
						"Output size does not match amount of output-neurons in trainingData set " + i);
			}
		}
	}
	 ...
	 * The training data has to consist of trainingsets, meaning arrays with an
	 * array of inputs in index 0 and an array of desired outputs in index
	 * 1.<br>
	 * This method groups trainingssets in mini-batches, which are evaluated
	 * after all sets and ragularization functions to reduce overfitting.<br>
	 * The costfunction has to be an instance of one of the local classes. The
	 * test data parameter should be alternative training data. The training
	 * will be evaluated on how good the results on the test data are, while
	 * training with the training data. For this reason the test data should not
	 * be the training data.<br>
	 * <br>
	 * <b>Pick these parameters with caution!</b> If one is off by too much the
	 * entire network might missfunction. This beeing said, the situation is
	 * different for every scenario. It takes time and experience to figure out
	 * how to set them.<br>
	 * These values might make a good start:
	 * <ul>
	 * <li>epochs: 15</li>
	 * <li>miniBatchSize: 10</li>
	 * <li>learningRate: 0.1</li>
	 * <li>regularization: 5</li>
	 * <li>costFunction: {@link BNN.NeuralNet.CrossEntropy CrossEntropy}</li>
	 * </ul>
	 *
	 * @param trainingData
	 *            array of trainingsets
	 * @param epochs
	 *            the amount of repetitions for the entire trainingData
	 * @param miniBatchSize
	 *            the amount of trainingSets trained before backpropagating
	 * @param learningRate
	 *            determines the speed with which the weights are adjusted
	 * @param regularization
	 *            defines how much small weights are prefered
	 * @param costFunction
	 *            the costfunction to use
	 * @param testData
	 *            data to test and evaluate the network on
	 */
	public void stochasticGradientDescent(double[][][] trainingData, int epochs, int miniBatchSize, double learningRate,
			double regularization, CostFunction costFunction, double[][][] testData) {
		stochasticGradientDescentCheckArguments(
				trainingData,
				epochs,
				miniBatchSize,
		...
		for (int i = 0; i < epochs; i++) {
			this.stochasticGradientDescentInner(
					trainingData,
					miniBatchSize,
					learningRate,
					regularization,
			...
			if (this.monitor.trainingAccuracy) {
				...
				System.out.print(
						String.format(
								"\n   Accuracy on training data:   %8d / %8d",
								trainingAccuracy[i],
								trainingData.length));
			}
			...
			if (this.monitor.evaluationAccuracy) {
				...
				System.out.print(
						String.format(
								"\n   Accuracy on evaluation data: %8d / %8d",
								evaluationAccuracy[i],
								testData.length));
			}
			System.out.println();
		}
		System.out.println(
				String.format("finished training in %5.8f seconds", (System.nanoTime() - startTime) / 1000000000));
	}
	 ...
	 * This method groups trainingssets in mini-batches, which are evaluated
	 * after all sets and ragularization functions to reduce overfitting.<br>
	 * The costfunction has to be an instance of one of the local classes.<br>
	 * <br>
	 * <b>Pick these parameters with caution!</b> If one is off by too much the
	 * entire network might missfunction. This beeing said, the situation is
	 * different for every scenario. It takes time and experience to figure out
	 ...
	 */
	public void stochasticGradientDescent(double[][][] trainingData, int epochs, int miniBatchSize, double learningRate,
			double regularization, CostFunction costFunction) {
		stochasticGradientDescentCheckArguments(
				trainingData,
				epochs,
				miniBatchSize,
	 ...
	 * This method groups trainingssets in mini-batches, which are evaluated
	 * after all sets and ragularization functions to reduce overfitting.<br>
	 * The test data parameter should be alternative training data. The training
	 * will be evaluated on how good the results on the test data are, while
	 * training with the training data. For this reason the test data should not
	 * be the training data.<br>
	 * <br>
	 ...
	 */
	public void stochasticGradientDescent(double[][][] trainingData, int epochs, int miniBatchSize, double learningRate,
			double regularization, double[][][] testData) {
	...
	protected void stochasticGradientDescentInner(double[][][] trainingData, int miniBatchSize, double learningRate,
			double regularization, CostFunction costFunction) {
	...
	protected void updateMiniBatch(double[][][] miniBatch, double learningRate, double regularization,
			double trainingDataLength, CostFunction costFunction) {
		...
		for (int j = this.weights.length - 1; j >= 0; j--) {
			this.weights[j] = VecMath.multiply(
					VecMath.subtract(
							this.weights[j],
	...
	protected double[][][][] backpropagate(double[][] touple, CostFunction costFunction) {
		...
		for (int i = this.sizes.length - 3; i >= 0; i--) {
			z = zVecs[i];
			delta = VecMath.multiply(
					VecMath.dot(VecMath.transpose(this.weights[i + 1]), delta),
					VecMath.sigmoidPrime(z));
			nablaBiases[i] = delta;
	 ...
	 * @param overwrite
	 *            wether to overwrite an existing file
	 * @throws IOException
	 *             if the file exists and is locked or an I/O error occours
	 */
